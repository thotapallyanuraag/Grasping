...
import matplotlib.pyplot as plt  # Importing Matplotlib for loss curve plotting
import warnings
...
def parse_args():
    ...
    parser.add_argument('--vis', action='store_true', help='Visualize the training process')

    args = parser.parse_args()
    return args


def save_predictions(images, preds, epoch, batch_idx, output_dir):
    """
    Save the predicted images with grasp rectangles overlaid.
    """
    os.makedirs(output_dir, exist_ok=True)

    for idx, (image, pred) in enumerate(zip(images, preds)):
        # Visualize or overlay predicted grasp rectangles (simple overlay for demonstration)
        pred_rects = pred['rectangles']  # Assuming 'rectangles' are part of your prediction output
        for rect in pred_rects:
            x, y, w, h = rect
            cv2.rectangle(image, (x, y), (x + w, y + h), (0, 255, 0), 2)

        # Save the result as an image
        filename = f"{output_dir}/epoch_{epoch}_batch_{batch_idx}_sample_{idx}.png"
...
def train(epoch, net, device, train_data, optimizer, batches_per_epoch, vis=False, output_dir="output_predictions", track_loss=True):
    """
    ...
    :param track_loss: Track loss values for plotting
    ...
    batch_idx = 0
    # Track loss values if track_loss is True
    epoch_losses = []
    ...
    while batch_idx < batches_per_epoch:
        for x, y, _, _, _ in train_data:
            ...
            if vis:
                save_predictions(x.cpu().numpy(), lossd['pred'], epoch, batch_idx, output_dir)

    # Plot loss curve after training
    if track_loss:
        plt.plot(epoch_losses)
        plt.title(f'Epoch {epoch} Loss Curve')
        plt.xlabel('Batch')
        plt.ylabel('Loss')
        plt.grid(True)
        plt.show()
...
def run():
    ...
    for epoch in range(args.epochs):
        ...
        train_results = train(epoch, net, device, train_data, optimizer, args.batches_per_epoch, vis=args.vis, output_dir="output_predictions")
